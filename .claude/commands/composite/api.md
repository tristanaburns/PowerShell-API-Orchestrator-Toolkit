...
Create a $1 API endpoint at $2 that handles $3
...
// Request Schema
interface $3Request {
  // Define based on $3 requirements
}
...
#### 1. Route Handler
```typescript
export async function handle$3(req: Request, res: Response) {
  try {
    // Input validation
    const validated = validate$3Input(req.body);
    
    // Authorization check
    const authorized = await checkPermissions(req.user, '$3');
    
    // Business logic
    const result = await process$3(validated);
