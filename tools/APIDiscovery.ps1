...
# OpenAPI discovery and endpoint exploration tool
...
try {
    ...
    if ($openApiSpec) {
        ...
        Write-Host "framework successfully discovered API structure!" -ForegroundColor Green
    }
    
    Write-Host ""
    Write-Host "Complete - API Discovery complete!" -ForegroundColor Green
}
catch {
    Write-Error "API Discovery failed: $_"
    exit 1
}